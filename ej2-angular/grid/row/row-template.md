...
> When using the `rowTemplate` feature in Syncfusion<sup style="font-size:70%">&reg;</sup> Angular Data Grid, keep in mind that any formatting applied to columns using the `format` property will not work inside the template.
...
The Syncfusion<sup style="font-size:70%">&reg;</sup> Angular Data Grid component provides flexibility to include custom controls such as Chart within the rows of the Grid. This feature enhances Grid interactivity by displaying graphical representations of data instead of plain text.
