...
The Data Grid allows customization of the height of rows based on needs. This can be achieved by using the [rowHeight](https://ej2.syncfusion.com/angular/documentation/api/grid/index-default#rowheight) property of the Data Grid component. This property allows changing the height of the entire grid row to a desired value.
...
> * It applies the specified height to all rows in the grid, including header and footer rows.
> *	Row-specific height customization requires the `rowDataBound` event approach.

### Customize specific row height

The row height for a particular row can be customized when displaying more content in a particular row, reducing the height of a row to fit content, or making a specific row stand out from the other rows in the grid. This can be achieved by using the [rowHeight](https://ej2.syncfusion.com/angular/documentation/api/grid/index-default#rowheight) property of the Data Grid component along with the [rowDataBound](https://ej2.syncfusion.com/angular/documentation/api/grid/index-default#rowdatabound) event. This event triggers every time a request is made to access row information, element, or data, and before the row element is appended to the grid element.
...
Row hover functionality provides visual feedback during mouse interaction, enhancing data readability and user experience. The [enableHover](https://ej2.syncfusion.com/angular/documentation/api/grid/index-default#enablehover) property controls hover effect availability across the entire Grid.
...
The [frozenRows](https://ej2.syncfusion.com/angular/documentation/api/grid/index-default#frozenrows) property in the Angular Data Grid is used to freeze a specified number of rows at the top of the grid, keeping them fixed while the rest of the content scrolls vertically.
...
The Data Grid provides the [addRecord](https://ej2.syncfusion.com/angular/documentation/api/grid/index-default#addrecord) method to programmatically add a new row. This feature is useful for inserting records without manual data entry.

The addRecord method takes two parameters:

* The `data` object representing the new row to be added
* The `index` at which the new row should be inserted. If no index is specified, the new row will be added at the end of the Data Grid.
...
## Show or hide rows programmatically

In the Data Grid, rows can be shown or hidden based on external actions, such as a checkbox click. This is useful for temporarily hiding rows without removing them from the underlying data source. This can be achieved by using the [getRowByIndex](https://ej2.syncfusion.com/angular/documentation/api/grid/index-default#getrowbyindex) and [getRowsObject](https://ej2.syncfusion.com/angular/documentation/api/grid/index-default#getrowsobject) methods of the grid along with the [change](https://ej2.syncfusion.com/angular/documentation/api/check-box/index-default#change) event of the checkbox.
...
In the following example, the "onCheckBoxChange" method checks whether the checkbox is selected. When selected, it iterates through all rows in the grid using the `getRowsObject` method. For each row, it checks whether the value in the CustomerID column is equal to "VINET". If a match is found, the row index is retrieved from the row object, and the corresponding row element is obtained using the `getRowByIndex` method. The row is then hidden by setting its display style to "none". The index of each hidden row is stored in the "hiddenRows" array.
...
## Get the row data and elements

The Data Grid provides several methods for retrieving row data and elements. These methods are useful for accessing specific rows, performing custom operations, and manipulating displayed data.

### Get row elements

1. [getRowByIndex](https://ej2.syncfusion.com/angular/documentation/api/grid/index-default#getrowbyindex): This method returns the HTML element of a row at the specified index. It can be used to retrieve the element of a specific row in the grid.

    ```ts
    const rowElement = this.grid.getRowByIndex(rowIndex); // Using the Data Grid component instance
    ```

2. [getRows](https://ej2.syncfusion.com/documentation/api/grid/index-default#getrows): This method returns an array of all the row elements in the grid. To retrieve both row data and row elements, the `getRows` method can be combined with the `getRowInfo` method.

    ```ts
     const rowElements = this.grid.getRows(); // Using the Data Grid component instance
    ```

3. [getSelectedRows](https://ej2.syncfusion.com/documentation/api/grid/index-default#getselectedrows): This method returns an array of HTML elements representing the selected rows in the grid. By iterating over this array, each row element and its data can be accessed using the `getRowInfo` method. In this way, both the row elements and their associated data for the selected rows can be retrieved.

    ```ts
    const selectedRowElements = this.grid.getSelectedRows(); // Using the Data Grid component instance
    ```

### Get row information

1. [getRowInfo](https://ej2.syncfusion.com/documentation/api/grid/index-default#getrowinfo): This method allows row information to be retrieved based on a cell target element.

    ```ts
    const rowInformation = this.grid.getRowInfo(targetElement); // Using the Data Grid component instance
    ```

2. [getSelectedRowIndexes](https://ej2.syncfusion.com/documentation/api/grid/index-default#getselectedrowindexes): This method allows the collection of indexes of the selected rows to be retrieved. However, it does not directly provide the row elements or the associated data. To access both the row elements and the data of the selected rows, the `getSelectedRowIndexes` method can be combined with the `getRowByIndex` and `getRowInfo` methods.

    ```ts
    const selectedIndexes = this.grid.getSelectedRowIndexes(); // Using the Data Grid component instance
    ```

3. [getRowIndexByPrimaryKey](https://ej2.syncfusion.com/documentation/api/grid/index-default#getrowindexbyprimarykey): This method allows the row index to be retrieved based on a specific primary key value or row data.

    ```ts
    const rowIndex = this.grid.getRowIndexByPrimaryKey(primaryKey); // Using the Data Grid component instance
    ```

These methods can be combined for comprehensive row data access and manipulation scenarios, providing complete control over Data Grid row interactions and customization.

## See Also

* [Customizing row height in Angular Data Grid](https://www.syncfusion.com/forums/138140/how-to-customize-the-row-height-in-angular-grid)
* [Setting font size and padding of Angular Data Grid's toolbar and filter bar](https://www.syncfusion.com/forums/150181/how-to-set-font-size-and-padding-of-angular-grids-toolbar-and-filter-bar)
* [Displaying serial number in Angular Data Grid](https://www.syncfusion.com/forums/166563/how-to-displaying-serial-number-in-angular-grid)
* [Adding/updating a new row programmatically in Angular Data Grid](https://www.syncfusion.com/forums/144356/how-to-add-update-a-new-row-programmatically-in-angular-grid)