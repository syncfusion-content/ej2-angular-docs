---
layout: post
title: Angular Grid Custom Data Binding | Syncfusion
description: Learn custom data binding in Angular Data Grid with dataStateChange, dataSourceChanged, observables, server-side ops, and APIs for efficient updates.
...
# Custom Data Binding in Angular Data Grid
...
The [Data Grid](https://www.syncfusion.com/angular-components/angular-data-grid) component offers a range of powerful features such as **paging**, **grouping**, **sorting**, and **filtering**. These actions trigger the [dataStateChange](https://ej2.syncfusion.com/angular/documentation/api/grid/index-default#datastatechange) event. CRUD actions such as **Create**, **Read**, **Update**, and **Delete** trigger the [dataSourceChanged](https://ej2.syncfusion.com/angular/documentation/api/grid/index-default#datasourcechanged) event. These events provide an opportunity to manage and manipulate data according to user interactions.
...
The Syncfusion Data Grid integrates with observables through the async pipe, which binds the latest emitted data to the grid. The [AsyncPipe](https://angular.dev/api/common/AsyncPipe) automatically subscribes to observables, extracting the latest emitted value with the required `result` and `count` properties expected by the grid.

The Data Grid supports handling of grid actions including **searching**, **filtering**, **sorting**, **grouping**, and **paging**. These interactions trigger the [dataStateChange](https://ej2.syncfusion.com/angular/documentation/api/grid/index-default#datastatechange) event, providing opportunities to manage and manipulate data according to grid interactions.
...
When a filtering operation is performed in the grid, the `dataStateChange` event is triggered, providing access to the following referenced arguments within the event.
...
When a grouping operation is performed in the grid, the `dataStateChange` event is triggered, providing access to the following referenced arguments within the event.
...
In Angular, lazy loading refers to the technique of loading data dynamically when it is needed, instead of loading everything upfront. Lazy load grouping enables efficient loading and display of grouped data by fetching only the required data on demand.
...
The complete example is available in the [handling CRUD operations topic](#handling-crud).
...
To implement CRUD operations using [Data Grid](https://www.syncfusion.com/angular-components/angular-data-grid), follow these steps:
...
4. **Call endEdit method:** After performing CRUD operations (adding, editing, or deleting), call the [endEdit](https://ej2.syncfusion.com/angular/documentation/api/grid/index-default#endedit) method to signal operation completion and update the grid accordingly.
...
> * Refer to the [guidelines for performing CRUD operations using observables](https://www.youtube.com/watch?v=yGLdi_Es0ac)
...
To implement CRUD operations using Data Grid, follow these steps:
...
Exporting all records is especially beneficial when dealing with large datasets that need to be exported for offline analysis or sharing.

By default, the Syncfusion Data Grid component exports only the records available on the current page. However, the Data Grid component also supports exporting all records—including those spanning multiple pages—by configuring the [pdfExportProperties](https://ej2.syncfusion.com/angular/documentation/api/grid/pdfExportProperties) and [excelExportProperties](https://ej2.syncfusion.com/angular/documentation/api/grid/excelExportProperties).
