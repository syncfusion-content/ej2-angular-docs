---
layout: post
title: Getting Started with Angular Input Mask | Syncfusion
description: Step-by-step setup for the Syncfusion Angular Input Mask, covering Angular CLI scaffolding, package install, theming, and a first masked input.
...
# Getting Started with Angular Input Mask

The Angular Input Mask is a textbox extended component that allows the user to enter only the valid input based on the provided mask. It is used to get the input with specific formats like Phone number, Date, Time, IP, Social security number, etc.

This guide provides step-by-step instructions for setting up an Angular project with TypeScript using Angular CLI, and integrating Syncfusion<sup style="font-size:70%">&reg;</sup> Angular Input Mask component. The recommended setup below targets modern Angular (CLI `ng add`). For module-based or framework host setups, see [See Also](#see-also).
...
## Adding the Syncfusion<sup style="font-size:70%">&reg;</sup> Angular Input Mask package

To install the **Syncfusion<sup style="font-size:70%">&reg;</sup> Angular Input Mask** package, use the following command:
...
## Add Syncfusion<sup style="font-size:70%">&reg;</sup> Angular Input Mask component

After package and theme setup, update the root component. File name and class name can vary by Angular CLI version (`src/app/app.ts` with `export class App`, or `app.component.ts` with `AppComponent`). Replace the root component content with the sample below, or merge the Angular Input Mask import, template, and data into your generated file.

> For the full property list, see the [Angular Input Mask API reference](https://ej2.syncfusion.com/angular/documentation/api/maskedtextbox/index-default).
...
When the build succeeds, the CLI reports a local URL (default: http://localhost:4200). Open that URL in a browser to view the Angular Input Mask. If the port is already in use, the CLI prompts for another port, or you can run `ng serve --port 4201`.
