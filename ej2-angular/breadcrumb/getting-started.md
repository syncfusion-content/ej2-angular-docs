...
## Adding the Syncfusion<sup style="font-size:70%">&reg;</sup> Angular Breadcrumb package

To install the **Syncfusion<sup style="font-size:70%">&reg;</sup> Angular Breadcrumb** package, use the following command:
...
When the build succeeds, the CLI reports a local URL (default: http://localhost:4200). Open that URL in a browser to view the Breadcrumb. If the port is already in use, the CLI prompts for another port, or you can run `ng serve --port 4201`
