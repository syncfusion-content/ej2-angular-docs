...
> This sample is hosted in a different location, so the Breadcrumb component renders with the sample environment location instead of the actual browser location.
...
The breadcrumb item text can be customized by using the `beforeItemRender` event. In the following example, `bind-to-location` text was customized as `location`.
