...
After package and theme setup, update the root component. File name and class name can vary by Angular CLI version (`src/app/app.ts` with `export class App`, or `app.component.ts` with `AppComponent`). Replace the root component content with the sample below, or merge the Button import, template, and data into your generated file.

> For the full property list, see the [Button API reference](https://ej2.syncfusion.com/angular/documentation/api/button/index-default).
