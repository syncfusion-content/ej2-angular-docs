...
This example demonstrates how to filter Syncfusion<sup style="font-size:70%">&reg;</sup> Angular ListBox data based on input from a TextBox. Bind an input event listener to the TextBox to capture user input and filter the items in the Angular ListBox. Within the event handler, use the [`filter`](https://ej2.syncfusion.com/angular/documentation/api/list-box/#filter) method to update the Angular ListBox items, ensuring that only those matching the input text are included.
