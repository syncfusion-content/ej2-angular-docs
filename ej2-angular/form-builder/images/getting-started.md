...
After package and theme setup, update the root component. File and class names can vary by Angular CLI version (`src/app/app.ts` with `export class App`, or `app.component.ts` with `AppComponent`). Replace the root component content with the sample below, or merge the Form Builder import, schema, and event handler into your generated file.

The Form Builder is a schema-driven component. Define your form by passing a JSON schema to the `schema` property of the `<ej2-form-builder>` element, and capture the submitted form data through the `submit` event.
