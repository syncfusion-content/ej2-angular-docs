...
`Note:` File restrictions apply when uploading images to the Angular Image Editor, whether through the open method or the built-in uploader. If `uploadSettings` is not defined in the sample, the Angular Image Editor, by default, allows files with `.jpg`, `.png`, `.svg`, `.webp`, and `.bmp` extensions, without any file size restrictions.
