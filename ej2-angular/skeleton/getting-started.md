...
## Add Syncfusion<sup style="font-size:70%">&reg;</sup> Skeleton component

After package and theme setup, update the root component. File name and class name can vary by Angular CLI version (`src/app/app.ts` with `export class App`, or `app.component.ts` with `AppComponent`). Replace the root component content with the sample below, or merge the Skeleton import, template, and data into your generated file.

> For the full property list, see the [Skeleton API reference](https://ej2.syncfusion.com/angular/documentation/api/skeleton/index-default).
...
When the build succeeds, the CLI reports a local URL (default: http://localhost:4200). Open that URL in a browser to view the Skeleton. If the port is already in use, the CLI prompts for another port, or you can run `ng serve --port 4201`.
