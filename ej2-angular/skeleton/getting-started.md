...
The Angular Skeleton component displays placeholder elements that mimic the layout of content while data is loading, providing users with a visual preview of the interface and improving the perceived loading experience.

This guide provides step-by-step instructions for setting up an Angular project with TypeScript using Angular CLI, and integrating Syncfusion<sup style="font-size:70%">&reg;</sup> Angular Skeleton component. The recommended setup below targets modern Angular (CLI `ng add`). For module-based or framework host setups, see [See Also](#see-also).
...
## Adding the Syncfusion<sup style="font-size:70%">&reg;</sup> Angular Skeleton package

To install the **Syncfusion<sup style="font-size:70%">&reg;</sup> Angular Skeleton** package, use the following command:
...
## Add Syncfusion<sup style="font-size:70%">&reg;</sup> Angular Skeleton component

After package and theme setup, update the root component. File name and class name can vary by Angular CLI version (`src/app/app.ts` with `export class App`, or `app.component.ts` with `AppComponent`). Replace the root component content with the sample below, or merge the Angular Skeleton import, template, and data into your generated file.

> For the full property list, see the [Angular Skeleton API reference](https://ej2.syncfusion.com/angular/documentation/api/skeleton/index-default).
...
When the build succeeds, the CLI reports a local URL (default: http://localhost:4200). Open that URL in a browser to view the Angular Skeleton. If the port is already in use, the CLI prompts for another port, or you can run `ng serve --port 4201`.
