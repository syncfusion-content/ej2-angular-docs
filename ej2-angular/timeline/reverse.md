...
The Angular Timeline component allows you to display items in reverse order by setting the [reverse](../api/timeline#reverse) property to `true`. This boolean property reverses the sequence of timeline items, making the most recent events appear first. This feature is especially useful for reverse timelines, activity feeds, news timelines, or any scenario where the latest information should be displayed.
