...
A cascading  Angular MultiSelect Dropdown consists of a series of dependent components, where the selection in one  Angular MultiSelect Dropdown (the parent) determines the available options in another (the child). This is useful for guiding users through hierarchical data, such as selecting a country to populate a list of states, which in turn populates a list of cities.
...
The following example demonstrates the cascade behavior of country, state, and city  Angular MultiSelect Dropdown components.
