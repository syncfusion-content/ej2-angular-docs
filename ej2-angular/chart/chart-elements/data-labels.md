...
Data labels display the value or custom text for each data point directly on the chart, making it easy to read exact values without relying on axes or tooltips.

<img src="../../images/datalabel.png" alt="Yes">
...
Label content can be formatted by using the template option. Inside the template, you can add the placeholder text `${point.x}` and `${point.y}` to display corresponding data points x & y value. To change color for the background in the datalabel template, you can use `${point.text}`.
