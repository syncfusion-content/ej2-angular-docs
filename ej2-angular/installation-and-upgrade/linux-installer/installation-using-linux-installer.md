...
4. To use the pre-packaged npm archives in an offline project, point npm at the bundled `npm` folder using the `--registry` or local file path syntax. For example, to install from a local tarball:
