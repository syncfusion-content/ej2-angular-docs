...
This article demonstrates how to dynamically load list items into a child list based on parent item selection in the Angular ListView component.
...
1. Initially, render the Angular ListView with the required data source.

2. Bind the [`select`](https://ej2.syncfusion.com/angular/documentation/api/list-view#select) event that triggers selecting list item in the Angular ListView component. By using the select event, you can push the new list item to the child list of the data source on specifying its item index. Item index can be obtained from the [`SelectEventArgs`](https://ej2.syncfusion.com/angular/documentation/api/list-view/selectEventArgs) of the select event.
{% tabs %}
