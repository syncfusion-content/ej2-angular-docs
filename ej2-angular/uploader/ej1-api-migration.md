...
| Get the files in sorted form | Not Applicable | **Method:** sortFileList(files: FileInfo[]): FileInfo[]<br/>  `<ejs-uploader #defaultupload></ejs-uploader>` <br/>  **TS:** <br/>  @ViewChild('defaultupload') public uploadObj: UploaderComponent; uploadObj.sortFileList(files)  |
...
| Change drop area text | **Property:** dropAreaText<br/> `<ej-uploadbox dropAreaText= 'Drop files here'></ej-uploadbox>`  | No separate property to change dropAreaText. It can be customized using locale texts |
...
| Show files already uploaded to the server | **Not Applicable** | **Property:** files <br/>`<ejs-uploader [files]='preloadFiles'></ejs-uploader>`<br/>  **TS:**<br/> public preloadFiles: any = [{name: 'nature', size: 5000,type: '.png'}] |
...
| Remove URL | **Property:** removeUrl<br/> `<ej-uploadbox [removeUrl]=' removeUrl'></ej-uploadbox>` | **Property:** asyncSettings.removeUrl<br/> `<ejs-uploader [asyncSettings]='path'></ejs-uploader>`<br/> `public path: Object = { removeUrl: 'https://ej2.syncfusion.com/services/api/uploadbox/Remove' };`  |
...
| Synchronous upload | **Property:** asyncUpload<br/> `<ej-uploadbox [asyncUpload]='false'></ej-uploadbox>`  | No separate property for enabling synchronous upload. It can be enabled using the configuration below <br/>`<ejs-uploader [autoUpload]='false'></ejs-uploader>`  |
| Key to get the selected files in server side | **Property:**  uploadName<br/> `<ej-uploadbox [uploadName]='UploadKey'></ej-uploadbox>`  | The id of the element is used as the key value |
| Upload the files dynamically | **Not Applicable** | **Method:** upload()<br/> `<ejs-uploader [autoUpload]='false' #upload></ejs-uploader>`<br/>  **TS:** <br/>  @ViewChild('upload') public uploadObj: UploaderComponent;<br/> public onUploadClick() { this.uploadObj.upload(filesData); } |
| Event triggers before start to upload the action | **Event:** beforeSend<br/> `<ej-uploadbox (beforeSend)= 'onBeforeSend ($event)'></ej-uploadbox>`<br/>  **TS:**<br/> public onBeforeSend: any= () => { }; | **Event:** uploading<br/> `<ejs-uploader (uploading)='beforeUploadStart ($event)'></ejs-uploader>`<br/>  **TS:**<br/> public beforeUploadStart: EmitType<UploadingEventArgs>= () => { };  |
| Event triggers when the upload is in progress | **Event:** inProgress<br/> `<ej-uploadbox (inProgress)= 'uploadInprogress($event)'></ej-uploadbox>`<br/>  **TS:**<br/> public uploadInprogress: any= () => { }; | **Event:** progress<br/> `<ejs-uploader #defaultupload  (progress)='uploadInprogress($event)'></ejs-uploader>`<br/>  **TS:**<br/> public uploadInprogress: EmitType<ProgressEventArgs>= () => { };  |
| Event triggers when upload got success | **Event:** success<br/> `<ej-uploadbox (success)= 'uploadSuccess($event)'></ej-uploadbox>`<br/>   **TS:**<br/> public uploadSuccess: any= () => { }; | **Event:** success<br/> `<ejs-uploader (success)='uploadSuccess($event)'></ejs-uploader>`<br/>  **TS:**<br/> public uploadSuccess: EmitType<SuccessEventArgs>= () => { }; |
| Event triggers when upload got failed | **Event:** error<br/> `<ej-uploadbox (error)= 'onUploadError($event)'></ej-uploadbox>`<br/>   **TS:** <br/>public onUploadError: any= () => { }; | **Event:** failure<br/> `<ejs-uploader (failure)='uploadFailure($event)'></ejs-uploader>`<br/>  **TS:**<br/> public uploadFailure: EmitType<FailureEventArgs>= () => { }; |
...
| Event triggers when cancel the upload | **Event:** cancel<br/> `<ej-uploadbox (cancel)= 'onUploadCancel($event)'></ej-uploadbox>` <br/>  **TS:**<br/> public onUploadCancel: any= () => { }; | **Event:** canceling<br/> `<ejs-uploader (canceling)='uploadingCancel($event)'></ejs-uploader>`<br/>  **TS:**<br/> public uploadingCancel: EmitType<CancelingEventArgs>= () => { };  |
...
| Retry the upload automatically when it fails | Not Applicable | **Property:** asyncSettings.retryCount,         asyncSettings.retryAfterDelay<br/> `<ejs-uploader [asyncSettings]='path'></ejs-uploader>`<br/>  **TS:**<br/> public path: Object = {  saveUrl: 'saveUrl',chunkSize: 50000,retryCount: 3,retryAfterDelay: 1000}; |
| Pause an uploading file | Not Applicable | **Method:** pause<br/> `<ejs-uploader #defaultupload></ejs-uploader>`<br/>  **TS:**<br/> @ViewChild('defaultupload') public uploadObj: UploaderComponent;   uploadObj.pause(filesData); |
...
| Resume a paused file | Not Applicable | **Method:** resume<br/> `<ejs-uploader #defaultupload></ejs-uploader>`<br/>  **TS:**<br/> @ViewChild('defaultupload')    public uploadObj: UploaderComponent;   uploadObj.resume(filesData); |
...
| Event triggers when the file removing succeed | **Event:** remove<br/> `<ej-uploadbox (remove)='onRemove($event)'></ej-uploadbox>`<br/>  **TS:**<br/> public onRemove: any= () => { }; | **Event:** success<br/> `<ejs-uploader (success)='onSuccess($event)'></ejs-uploader>`<br/>  **TS:**<br/> public onSuccess: EmitType<SuccessEventArgs>= () => { }; |
| Event triggers when the file removing fails | **Not Applicable** | **Event:** failure<br/> `<ejs-uploader (failure)='onFailure($event)'></ejs-uploader>`<br/>  **TS:**<br/> public onFailure: EmitType<FailureEventArgs>= () => { }; |
...
| **Behavior** | **Property in Essential<sup style="font-size:70%">&reg;</sup> JS  1** | **Property in Essential<sup style="font-size:70%">&reg;</sup> JS 2** |
| ------------ | --------------------------------| ------------------------------ |
| Enable drag and drop upload | **Property:** allowDragAndDrop<br/> `<ej-uploadbox [allowDragAndDrop]='true'></ej-uploadbox>` | Drag-and-drop is enabled by default; no separate property is required to enable it |
...
| Enable/Disable the control | **Property:** enabled<br/> `<ej-uploadbox id='uploadBox' [enabled]='false'></ej-uploadbox>`  **Method:** enable(), disable()<br/> $('#uploadBox').ejUploadbox('enable');   $('#uploadBox').ejUploadbox('disable');   | **Property:** enabled<br/> `<ejs-uploader [enabled]='false'></ejs-uploader>` (EJ2 has no `enable()`/`disable()` methods; set the `enabled` property instead) |
...
| Get the selected files data | **Not Applicable** | **Method:** getFilesData(): FileInfo[]<br/> `<ejs-uploader #defaultupload></ejs-uploader>`<br/>  **TS:**<br/> @ViewChild('defaultupload')  public uploadObj: UploaderComponent;  uploadObj.getFilesData();  |
| Convert bytes in to MB, GB | **Not Applicable** | **Method:** bytesToSize(bytes: number): string<br/> `<ejs-uploader #defaultupload></ejs-uploader>`<br/>  **TS:**<br/> @ViewChild('defaultupload')  public uploadObj: UploaderComponent;    uploadObj.bytesToSize(5000); |