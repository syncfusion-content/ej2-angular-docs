...
The Localization library allows you to localize the default text content of the Angular Stepper. You can change the static text content used for the `optional` property to other cultures (Arabic, Deutsch, French, etc.) by defining the `locale` value and its translation object.

| Locale key | en-US (default) |
|-----|-----|
| optional | Optional |

In this example, the `French` culture is set to Angular Stepper and the default text is updated with the content defined by the locale key.
...
RTL provides an option to switch the text direction and layout of the Angular Stepper component from right to left by setting the [enableRtl](https://ej2.syncfusion.com/angular/documentation/api/stepper#enablertl) property to `true`.
