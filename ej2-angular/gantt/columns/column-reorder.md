...
The Syncfusion Angular Gantt chart component allows to reorder columns by drag and drop of a particular column header from one index to another index within the gantt chart. This feature can be enabled by injecting the **ReorderService** in the provider section of your **AppModule**.
...
The Syncfusion Angular Gantt chart allows you to reorder columns externally, which means that using methods you can programmatically move columns around within the gantt chart, based on their index or target index, or by using their field name.
...
When reordering columns in the Syncfusion Angular Gantt chart component, you may want to take some specific action in response to the drag and drop events. To handle these events, you can define event handlers for the following events:
...
In the following example, we have implemented the `columnDragStart`, `columnDrag`, and `columnDrop` events in the Syncfusion Gantt chart component.
