...
description: Learn how to get started with the Syncfusion Angular Speed Dial component, including setup, installation, and basic configuration.
...
The following dependencies are required to use the Speed Dial component in your application.
...
To install the **Syncfusion<sup style="font-size:70%">&reg;</sup> Angular Speed Dial** package, use the following command:
...
Themes for the Syncfusion<sup style="font-size:70%">&reg;</sup> Speed Dial component can be applied using CSS files provided through [npm theme packages](https://www.npmjs.com/package/@syncfusion/ej2-material3-theme). For available themes, refer to the [Themes](https://ej2.syncfusion.com/angular/documentation/appearance/overview) documentation.
...
## Add Syncfusion<sup style="font-size:70%">&reg;</sup> Speed Dial component

After package and theme setup, update the root component. File name and class name can vary by Angular CLI version (`src/app/app.ts` with `export class App`, or `app.component.ts` with `AppComponent`). Replace the root component content with the sample below, or merge the Speed Dial import, template, and data into your generated file.

> For the full property list, see the [Speed Dial API reference](https://ej2.syncfusion.com/angular/documentation/api/speed-dial).
...
export class App {
  public items: SpeedDialItemModel[] = [{ text: 'Cut' }, { text: 'Copy' }, { text: 'Paste' }];
}
```
Place the following markup in the component template file referenced by `templateUrl` (for example, `src/app/app.html`):

```html
<button ejs-speeddial content='Edit' [items]='items'></button>
```
...
When the build succeeds, the CLI reports a local URL (default: http://localhost:4200). Open that URL in a browser to view the Speed Dial. If the port is already in use, the CLI prompts for another port, or you can run `ng serve --port 4201`.
