...
description: Learn how to get started with the Syncfusion Angular Floating Action Button component, including setup, installation, and basic configuration.
...
The following dependencies are required to use the Floating Action Button component in your application.
...
To install the **Syncfusion<sup style="font-size:70%">&reg;</sup> Angular Floating Action Button** package, use the following command:
...
Themes for the Syncfusion<sup style="font-size:70%">&reg;</sup> Floating Action Button component can be applied using CSS files provided through [npm theme packages](https://www.npmjs.com/package/@syncfusion/ej2-material3-theme). For available themes, refer to the [Themes](https://ej2.syncfusion.com/angular/documentation/appearance/overview) documentation.
...
After package and theme setup, update the root component. File name and class name can vary by Angular CLI version (`src/app/app.ts` with `export class App`, or `app.component.ts` with `AppComponent`). Replace the root component content with the sample below, or merge the Floating Action Button import and template into your generated file.

> For the full property list, see the [Floating Action Button API reference](https://ej2.syncfusion.com/angular/documentation/api/floating-action-button/index-default).
...
When the build succeeds, the CLI reports a local URL (default: http://localhost:4200). Open that URL in a browser to view the Floating Action Button. If the port is already in use, the CLI prompts for another port, or you can run `ng serve --port 4201`.
