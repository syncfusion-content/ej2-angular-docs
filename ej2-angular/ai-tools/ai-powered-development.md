...
- Use `providers: [PageService, SortService, ...]` in @Component decorator
- Import from @syncfusion/ej2-angular-[package]
- If unsure about API details, search the documentation at https://ej2.syncfusion.com/angular/documentation/
```
...
A generic request like "Create a Syncfusion Angular Grid" generates code that fails at runtime—wrong imports, missing service configuration, unsupported properties.

**With documentation links:**
When you include the specific docs URL, the AI generates correct code: proper imports, `<ColumnsDirective>` structure, required `providers: [PageService, SortService, FilterService]` in `@Component` decorator, CSS, and all necessary details.
...
|---|---|---|
| Component feature doesn't work (e.g. sorting, filtering) | Missing `providers` for required service | Add `providers: [PageService, SortService, ...]` in `@Component` decorator |
...
| `providers` not declared in component | Missing service injection | Add required services to `providers: [...]` array in `@Component` decorator |
