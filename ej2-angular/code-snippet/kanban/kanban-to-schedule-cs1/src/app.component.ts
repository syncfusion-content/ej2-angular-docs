...
import { ScheduleModule, RecurrenceEditorAllModule, } from '@syncfusion/ej2-angular-schedule';
...
import { EventSettingsModel, TimelineViewsService, TimelineMonthService, ResizeService, DragAndDropService, ScheduleComponent,
    View, WorkHoursModel, GroupModel, ResourceDetails, ResourcesModel,
} from '@syncfusion/ej2-angular-schedule';

@Component({
    imports: [KanbanModule, ScheduleModule, RecurrenceEditorAllModule],
    standalone: true,
    selector: 'app-root',
...
export class AppComponent {
    ...
    public scheduleDataSource: Object[] = <Object[]>(
      extend([], scheduleData, undefined, true)
    );
    ...
    getConsultantName(value: ResourceDetails): string {
      return (value as ResourceDetails).resourceData[
        ((value as ResourceDetails).resource as ResourcesModel)
          .textField as string
      ] as string;
    }

    getConsultantStatus(value: ResourceDetails): boolean {
      let resourceName: string = (value as ResourceDetails).resourceData[
        ((value as ResourceDetails).resource as ResourcesModel)
          .textField as string
      ] as string;
    ...
    onKanbanDragStop(args: DragEventArgs) {
      let scheduleElement: Element = <Element>(
        closest(args.event.target as Element, '#Schedule')
      );
      if (scheduleElement) {
    ...
    scheduleDragStop(args: any) {
      ...
      if (kanbanElement) {
        ...
        removeClass(
          [
            (this.scheduleObj as ScheduleComponent).element.querySelector(
              '.e-selected-cell'
            ) as Element,
          ],
          'e-selected-cell'
