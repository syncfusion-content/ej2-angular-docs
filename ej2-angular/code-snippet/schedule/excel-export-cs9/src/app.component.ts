...
import { ScheduleComponent, EventSettingsModel, View, ActionEventArgs, ToolbarActionArgs, ExportOptions, ExcelExportService, ExcelExportEventArgs } from '@syncfusion/ej2-angular-schedule';
...
@Component({
  ...
  providers: [DayService, WeekService, WorkWeekService, MonthService, AgendaService, MonthAgendaService, ExcelExportService],
  standalone: true,
  selector: 'app-root',
  template: `
    <ejs-schedule #scheduleObj width='100%' height='550px' [views]="scheduleViews" cssClass='schedule-excel-export' 
      [selectedDate]="selectedDate" [eventSettings]="eventSettings" [currentView]="currentView" 
      (actionBegin)="onActionBegin($event)" (excelExport)="onExcelExport($event)">
      <e-views>
...
export class AppComponent {
  ...
  public onExcelExport(args: ExcelExportEventArgs): void {
    ...
    worksheet.rows.unshift({
      index: 1,
      cells: [{
        index: 1, value: 'Sales Report', colSpan: worksheet.columns.length,
    ...
    worksheet.rows.unshift({
      index: 2,
      cells: [{
        index: 1, value: 'Generated on: ' + new Date().toLocaleDateString(), colSpan: worksheet.columns.length,
    ...
    worksheet.rows.push({
      ...
      cells: [{
        index: 1, value: 'End of Report', colSpan: worksheet.columns.length,
