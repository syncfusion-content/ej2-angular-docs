...
@Component({
  ...
  template: `  
        <div style="display: flex; align-items: center; gap: 10px; margin-bottom: 10px;">
            ...
            <ejs-textbox #rowtextbox width="100"></ejs-textbox>
            <label style="font-weight: bold;">Enter the cell index:</label>
            <ejs-textbox #celltextbox width="100"></ejs-textbox>
...
export class AppComponent implements OnInit {
  ...
  @ViewChild('rowtextbox', { static: false }) public rowtextbox?: TextBoxComponent;
  @ViewChild('celltextbox', { static: false }) public celltextbox?: TextBoxComponent;
  public data: object[] = [];
  ...
  public click(): void {
    const rowIndexValue = (this.rowtextbox as TextBoxComponent).element.value as string;
    const cellIndexValue = (this.celltextbox as TextBoxComponent).element.value as string;
    const rowIndex = parseInt(rowIndexValue, 10);
    const cellIndex = parseInt(cellIndexValue, 10);
    if (!isNaN(cellIndex)) {
      this.ganttInstance.selectCell({
        rowIndex: rowIndex,
        cellIndex: cellIndex
      });
    }
