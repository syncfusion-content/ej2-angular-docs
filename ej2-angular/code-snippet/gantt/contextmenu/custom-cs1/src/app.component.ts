...
export class AppComponent implements OnInit {
  ...
  public contextMenuClick(args: ContextMenuClickEventArgs) {
    ...
    if (args.item.id === 'collapserow') {
      (this.ganttInstance as GanttComponent).collapseByID(Number(record!.ganttProperties!.taskId));
    }
    if (args.item.id === 'expandrow') {
      (this.ganttInstance as GanttComponent).expandByID(Number(record!.ganttProperties!.taskId));
    }
    if (args.item.id === 'hidecols') {
      (this.ganttInstance as GanttComponent).hideColumn(args.column!.headerText);
    }
  }
