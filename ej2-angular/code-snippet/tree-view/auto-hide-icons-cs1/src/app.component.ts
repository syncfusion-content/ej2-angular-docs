...
@Component({
  ...
  template: `<div id='treeparent'><ejs-treeview id='treeElement' #treevalidate [fields]='field' (mouseenter)="showIcons()" (mouseleave)="hideIcons()"></ejs-treeview></div>`,
})
...
export class AppComponent {
  ...
  hideIcons(): void {
    this.setIconsVisibility('hidden');
  }

  private setIconsVisibility(visibility: 'visible' | 'hidden'): void {
    if (this.tree && (this.tree as any).element) {
      const icons = (this.tree as any).element.querySelectorAll('.e-icons');
      icons.forEach((icon: Element) => {
        (icon as HTMLElement).style.visibility = visibility;
      });
    }
  }
}
