...
The **Angular In-place Editor** supports adding HTML5 input components using the [template](https://ej2.syncfusion.com/angular/documentation/api/inplace-editor#template) property. The Template property can be given as either a `string` or a `query selector`.
...
You can render other components inside **Angular In-place Editor** using Angular `ng-template`. We need to use `ng-template` inside the `<ejs-inplaceeditor>` tag with `#template` attribute, which is mandatory to render that template.
