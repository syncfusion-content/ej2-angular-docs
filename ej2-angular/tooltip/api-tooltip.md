...
It is used to display the Angular Tooltip and content of Tooltip from right to left direction.
...
We can trigger `beforeClose` event before the Angular Tooltip hides from the screen. If returned false, then the Tooltip is no more hidden.
