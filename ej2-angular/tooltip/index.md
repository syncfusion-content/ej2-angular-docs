---
layout: post
title: About Syncfusion Angular Tooltip Component | Syncfusion
description: Discover the Syncfusion Angular Tooltip, a popup that displays text, images, or templates on hover, focus, click, or touch for contextual guidance.
platform: ej2-angular
control: Tooltip 
documentation: ug
domainurl: ##DomainURL##
---

# About Syncfusion Angular Tooltip Component.

The **Angular Tooltip** component displays a pop-up containing an information or a message when you hover, click, focus, or touch an element.
The information displayed in the Angular Tooltip can include simple text, images, hyperlinks, or custom templates. In mobile devices, to display the Angular Tooltip, you need to tap and hold the target elements.

## Key features

Angular Tooltip
* Can be displayed in 12 different positions.
* Supports animation effects while showing/hiding the Angular Tooltip.
* Content can be assigned with static text, template, or loaded dynamically via AJAX.
* Supports 4 opening modes, `hover`, `click`, `focus`, and `custom`.
* Supports auto tip positioning.
* Can be moved along with the mouse pointer using the mouse trailing option.
* Can be opened in Sticky mode, allowing you to close/hide the Angular Tooltip manually.
